...
const AdminFilter = ({ filterOpen,searchParams,searchQuery,setFilterOpen }) => {
        ...
        >
          ...
          <div className="mt-4 w-full flex flex-col gap-y-3">
            {filters?.map((item, idx) => (
              ...
              >
                <div className="text-[12px] sm:text-[18px]">{item?.name || ""}</div>
                <div><Checkbox checked={item?.selected}/></div>
              </div>
            ))}
          </div>
          <div className="w-full mt-5 flex justify-center">
            <Button className="text-[12px] sm:text-[16px]" onClick={()=>applyFilters()}>Apply Filters</Button>
          </div>
        </SheetContent>
      </Sheet>
    </div>
